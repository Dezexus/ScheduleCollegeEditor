...
{
    class ExportManHoursGroupByLessonAndGroup
    {
        ...
        private static async void FillTeacherInWeek(Excel.Worksheet Sheet, Excel.Application excelApp) {
            ...
            Sheet.Cells[1, colum + 2] = "Группа";
            Sheet.Cells[1, colum + 3] = "Часы";
            ...
            foreach (var numberTeacherHours in await GetWeekForTeacher()) {
                foreach (var item in numberTeacherHours.CountHourForItems) {
                    ...
                    Sheet.Cells[row, colum + 2] = item.Group;
                    Sheet.Cells[row, colum + 3] = item.Hour;
                ...
                Sheet.Cells[row, colum + 3].Interior.Color = Excel.XlRgbColor.rgbLightGray;
                row++;
            }
        ...
        private static void ChangingAppearanceWeek(Excel.Worksheet Sheet, int colum, int row) {
            ...
            Sheet.Cells[1, colum + 1].ColumnWidth = 35;
            Sheet.Cells[1, colum + 1].ColumnWidth = 20;
            Sheet.Cells[1, colum + 3].ColumnWidth = 10;

            string startRange1 = ExcelFunc.GetColumnLetter(colum) + 1; //Get the end range
            string endRange1 = ExcelFunc.GetColumnLetter(colum + 3) + (row - 1); //Get the end range
        ...
        private static async Task<List<NumberTeacherHours>> GetWeekForTeacher() {
            ...
            var groups = schedules.Select(x => x._Group.Name).Distinct().ToList();
            ...
            foreach (var teacher in teachers) {
                ...
                foreach (var group in groups) {
                    ...
                    var schooleSubjects = schedules.Where(x => x.Teacher.FullName == teacher 
                        && x._Group.Name == group).OrderBy(x => x.SchoolSubject.Name).Select(x => x.SchoolSubject.Name).Distinct().ToList();
                    if (schooleSubjects.Count == 0)
                        continue;

                    foreach (var schooleSubject in schooleSubjects) {

                        int hours = schedules.Where(x => x.Teacher.FullName == teacher && x._Group.Name == group
        ...
        private static async Task<List<NumberTeacherHours>> GetDayForTeacher(string day) {
            ...
            var groups = schedules.Where(x => x._WeekDay.DayName == day).Select(
                x => x._Group.Name).Distinct().ToList();
            ...
            foreach (var teacher in teachers) {
                ...
                foreach (var group in groups) {
                    ...
                    var schooleSubjects = schedules.Where(x => x._WeekDay.DayName == day && x.Teacher.FullName == teacher 
                        && x._Group.Name == group).OrderBy(x => x.SchoolSubject.Name).Select(x => x.SchoolSubject.Name).Distinct().ToList();
                    if (schooleSubjects.Count == 0)
                        continue;
                    ...
                    foreach (var schooleSubject in schooleSubjects) {
                        ...
                        int hours = schedules.Where(x => x._WeekDay.DayName == day && x.Teacher.FullName == teacher && x._Group.Name == group
                            && x.SchoolSubject.Name == schooleSubject).Count() * 2;
